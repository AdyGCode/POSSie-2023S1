...
def test_word(word, guess, expected_scores):
    scores = get_letter_scores(word, guess)

    if scores == expected_scores:
        print(f"Result: As expected     | {word} | {guess} | {expected_scores} | {scores}")
    else:
        print(f"Result: Not as expected | {word} | {guess} | {expected_scores} | {scores}")


#          WORD     GUESS    EXPECTED SCORES
test_word("hello", "hello", [2, 2, 2, 2, 2])
test_word("nears", "saner", [1, 1, 1, 1, 1])
test_word("hello", "spams", [0, 0, 0, 0, 0])

test_word('relit', 'tiler', [1, 1, 2, 1, 1])
test_word("hello", "allow", [0, 1, 2, 1, 0])
test_word("gauge", "range", [0, 2, 0, 2, 2])

test_word("melee", "erect", [1, 0, 1, 0, 0])
test_word("erect", "melee", [0, 1, 0, 1, 0])
test_word("melee", "meter", [2, 2, 0, 2, 0])
test_word("meter", "melee", [2, 2, 0, 2, 0])
test_word("meter", "erect", [1, 1, 1, 0, 1])
test_word("erect", "meter", [0, 1, 1, 1, 1])

test_word("drain", "float", [0, 0, 0, 1, 0])
test_word("array", "spray", [0, 0, 2, 2, 2])
test_word("train", "tenor", [2, 0, 1, 0, 1])
test_word("heals", "hello", [2, 2, 0, 2, 0])
test_word("alloy", "spray", [0, 0, 0, 1, 2])
test_word("alloy", "lorry", [1, 1, 0, 0, 2])
test_word("lorry", "alloy", [0, 1, 0, 1, 2])
test_word("array", "sprat", [0, 0, 2, 2, 0])
test_word("train", "inter", [1, 1, 1, 0, 1])

test_word("minor", "major", [2, 0, 0, 2, 2])
test_word("evens", "every", [2, 2, 2, 0, 0])
test_word("tyres", "stray", [1, 1, 2, 0, 1])

test_word("bores", "robes", [1, 2, 1, 2, 2])
test_word("sober", "robes", [1, 2, 2, 2, 1])
test_word("bores", "sober", [1, 2, 1, 2, 1])

test_word("petal", "level", [0, 2, 0, 0, 2])
